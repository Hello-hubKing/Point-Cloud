...
from utils import point_image
import data_utils as d_utils
import ModelNet40Loader
import shapenet_part_loader
from model_PFNet import _netlocalD,_netG,_netlocalD_deepimage
import numpy as np
...
if __name__ == '__main__':
    ...
    point_netD_deepimage = _netlocalD_deepimage(1,16)
    ...
    if USE_CUDA:
        ...
        point_netD_deepimage=torch.nn.DataParallel(point_netD_deepimage)
        ...
        point_netD_deepimage.to(device)
        point_netD_deepimage.apply(weights_init_normal)
    if opt.netG != '':
    ...
    if opt.netD != '':
        ...
        #深度图片辨别器
        point_netD_deepimage.load_state_dict(torch.load(opt.netD, map_location=lambda storage, location: storage)['state_dict'])
        resume_epoch = torch.load(opt.netD)['epoch']
    ...
    #深度图片辨别器
    optimizerD_deepimage = torch.optim.Adam(point_netD_deepimage.parameters(), lr=0.0001, betas=(0.9, 0.999), eps=1e-05,
                                  weight_decay=opt.weight_decay)
    ...
    schedulerD_deepimage = torch.optim.lr_scheduler.StepLR(optimizerD_deepimage, step_size=40, gamma=0.2)
    ...
    if opt.D_choose == 1:
        for epoch in range(resume_epoch, opt.niter):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                # 深度图片辨别器
                point_netD_deepimage = point_netD_deepimage.train()
                ...
                # 深度图片辨别器
                point_netD_deepimage.zero_grad()
                ...
                real_center_iamge=point_image(real_center)
                real_center_iamge=torch.unsqueeze(torch.tensor(real_center_iamge),1).type(torch.FloatTensor)

                # print(type(real_center_iamge))
                output_deepimage = point_netD_deepimage(real_center_iamge)

                errD_real = criterion(output, label)
                errD_real.backward()

                errD_real_deepimage = criterion(output_deepimage, label)
                errD_real_deepimage.backward()
                ...
                # output = point_netD_deepimage(fake.detach())

                fake_center_iamge=point_image(fake.detach())
                fake_center_iamge=torch.unsqueeze(torch.tensor(fake_center_iamge),1).type(torch.FloatTensor)
                output_deepimage = point_netD_deepimage(fake_center_iamge.detach())

                errD_fake = criterion(output, label)
                errD_fake.backward()
                errD_fake_deepimage = criterion(output_deepimage, label)
                errD_fake_deepimage.backward()


                errD = errD_real + errD_fake
                optimizerD.step()
                errD_deepimage = errD_real_deepimage + errD_fake_deepimage
                optimizerD_deepimage.step()
                ...
                fake_center_iamge = point_image(fake.detach())
                fake_center_iamge = torch.unsqueeze(torch.tensor(fake_center_iamge), 1).type(torch.FloatTensor)
                output_deepimage = point_netD_deepimage(fake_center_iamge)

                errG_D = criterion(output, label)+criterion(output_deepimage, label)
                errG_l2 = 0
            ...
            if epoch % 10 == 0:
                ...
                #深度图片
                torch.save({'epoch': epoch + 1,
                            'state_dict': point_netD_deepimage.state_dict()},
                           'Trained_Model/point_netD_deepimage' + str(epoch) + '.pth')
