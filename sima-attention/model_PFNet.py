...
class Convlayer(nn.Module):
    def __init__(self, point_scales):
        ...
        #Sima注意力
        attention_module = functools.partial(simam_module)
        self.conv3 = conv1x1(64, 128, 1)
        self.conv3 = nn.Sequential(self.conv3,attention_module(128))

        # self.conv4 = torch.nn.Conv2d(128, 256, 1)
        self.conv4 = conv1x1(128, 256, 1)
        self.conv4 = nn.Sequential(self.conv4, attention_module(256))

        # self.conv5 = torch.nn.Conv2d(256, 512, 1)
        self.conv5= conv1x1(256, 512, 1)
        self.conv5 = nn.Sequential(self.conv5, attention_module(512))

        # self.conv6 = torch.nn.Conv2d(512, 1024, 1)
        self.conv6= conv1x1(512, 1024, 1)
        self.conv6 = nn.Sequential(self.conv6, attention_module(1024))
